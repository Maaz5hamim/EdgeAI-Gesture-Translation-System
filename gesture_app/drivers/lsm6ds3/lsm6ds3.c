...
static int lsm6ds3_odr_to_freq_val(uint16_t odr)
{
	...
	BUILD_ASSERT(ARRAY_SIZE(lsm6ds3_odr_map) > 10);
	return lsm6ds3_odr_map[10];
}
...
static inline int lsm6ds3_reboot(const struct device *dev)
{
	...
	if (data->hw_tf->update_reg(dev, LSM6DS3_REG_CTRL3_C,
				    LSM6DS3_MASK_CTRL3_C_BOOT,
				    1 << LSM6DS3_SHIFT_CTRL3_C_BOOT) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_accel_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL1_XL,
				    LSM6DS3_MASK_CTRL1_XL_FS_XL,
				    fs << LSM6DS3_SHIFT_CTRL1_XL_FS_XL) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_accel_set_odr_raw(const struct device *dev, uint8_t odr)
{
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL1_XL,
				    LSM6DS3_MASK_CTRL1_XL_ODR_XL,
				    odr << LSM6DS3_SHIFT_CTRL1_XL_ODR_XL) < 0) {
		return -EIO;
	}
...
static int lsm6ds3_gyro_set_fs_raw(const struct device *dev, uint8_t fs)
{
	...
	if (fs == GYRO_FULLSCALE_125) {
		if (data->hw_tf->update_reg(dev,
					LSM6DS3_REG_CTRL2_G,
					LSM6DS3_MASK_CTRL2_FS125 | LSM6DS3_MASK_CTRL2_G_FS_G,
					1 << LSM6DS3_SHIFT_CTRL2_FS125) < 0) {
			return -EIO;
		}
	} else {
		if (data->hw_tf->update_reg(dev,
					LSM6DS3_REG_CTRL2_G,
					LSM6DS3_MASK_CTRL2_FS125 | LSM6DS3_MASK_CTRL2_G_FS_G,
					fs << LSM6DS3_SHIFT_CTRL2_G_FS_G) < 0) {
			return -EIO;
		}
	}
...
static int lsm6ds3_gyro_set_odr_raw(const struct device *dev, uint8_t odr)
{
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL2_G,
				    LSM6DS3_MASK_CTRL2_G_ODR_G,
				    odr << LSM6DS3_SHIFT_CTRL2_G_ODR_G) < 0) {
		return -EIO;
	}

	data->gyro_freq = lsm6ds3_odr_to_freq_val(odr);
...
static int lsm6ds3_accel_range_set(const struct device *dev, int32_t range)
{
	...
	data->accel_sensitivity = (float)(lsm6ds3_accel_fs_sens[fs]
						    * SENSI_GRAIN_XL);
	return 0;
}
#endif
...
static int lsm6ds3_gyro_range_set(const struct device *dev, int32_t range)
{
	...
	data->gyro_sensitivity = (float)(lsm6ds3_gyro_fs_sens[fs]
						    * SENSI_GRAIN_G);
	return 0;
}
#endif
...
static int lsm6ds3_sample_fetch_accel(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DS3_REG_OUTX_L_XL,
				   buf, sizeof(buf)) < 0) {
...
static int lsm6ds3_sample_fetch_gyro(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DS3_REG_OUTX_L_G,
				   buf, sizeof(buf)) < 0) {
		LOG_DBG("failed to read sample");
		return -EIO;
	}
...
static int lsm6ds3_sample_fetch_temp(const struct device *dev)
{
	...
	if (data->hw_tf->read_data(dev, LSM6DS3_REG_OUT_TEMP_L,
				   buf, sizeof(buf)) < 0) {
		LOG_DBG("failed to read sample");
		return -EIO;
	}
...
#if defined(CONFIG_LSM6DS3_EXT0_LIS2MDL) || defined(CONFIG_LSM6DS3_EXT0_LIS3MDL)
					    ...
					    float sensitivity)
{
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		lsm6ds3_accel_convert(val, data->accel_sample_x, sensitivity);
		lsm6ds3_accel_convert(val + 1, data->accel_sample_y,
				      sensitivity);
		lsm6ds3_accel_convert(val + 2, data->accel_sample_z,
				      sensitivity);
		break;
	default:
		return -ENOTSUP;
...
static int lsm6ds3_accel_channel_get(enum sensor_channel chan,
				     ...
				     struct lsm6ds3_data *data)
{
	return lsm6ds3_accel_get_channel(chan, val, data,
					   ...
					   struct lsm6ds3_data *data)
{
	...
	case SENSOR_CHAN_MAGN_XYZ:
		lsm6ds3_magn_convert(val,
				     data->magn_sample_x,
				     data->magn_sensitivity);
		lsm6ds3_magn_convert(val + 1,
				     data->magn_sample_y,
				     data->magn_sensitivity);
		lsm6ds3_magn_convert(val + 2,
				     data->magn_sample_z,
				     data->magn_sensitivity);
		break;
	default:
...
static int lsm6ds3_magn_channel_get(enum sensor_channel chan,
				    struct sensor_value *val,
				    struct lsm6ds3_data *data)
{
...
static DEVICE_API(sensor, lsm6ds3_driver_api) = {
...
static int lsm6ds3_init_chip(const struct device *dev)
{
	...
	if (data->hw_tf->update_reg(dev,
				LSM6DS3_REG_FIFO_CTRL5,
				LSM6DS3_MASK_FIFO_CTRL5_FIFO_MODE,
				0 << LSM6DS3_SHIFT_FIFO_CTRL5_FIFO_MODE) < 0) {
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL3_C,
				    LSM6DS3_MASK_CTRL3_C_BDU |
				    LSM6DS3_MASK_CTRL3_C_BLE |
				    LSM6DS3_MASK_CTRL3_C_IF_INC,
				    (1 << LSM6DS3_SHIFT_CTRL3_C_BDU) |
				    (0 << LSM6DS3_SHIFT_CTRL3_C_BLE) |
				    (1 << LSM6DS3_SHIFT_CTRL3_C_IF_INC)) < 0) {
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL6_C,
				    LSM6DS3_MASK_CTRL6_C_XL_HM_MODE,
				    (1 << LSM6DS3_SHIFT_CTRL6_C_XL_HM_MODE)) < 0) {
	...
	if (data->hw_tf->update_reg(dev,
				    LSM6DS3_REG_CTRL7_G,
				    LSM6DS3_MASK_CTRL7_G_HM_MODE,
				    (1 << LSM6DS3_SHIFT_CTRL7_G_HM_MODE)) < 0) {
...
static int lsm6ds3_init(const struct device *dev)
{
	...
	ret = lsm6ds3_init_interrupt(dev);
	if (ret < 0) {
	...
	ret = lsm6ds3_shub_init_external_chip(dev);
	if (ret < 0) {
			     ...
			     enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		accel_odr = lsm6ds3_freq_to_odr_val(data->accel_freq);
		ret = lsm6ds3_accel_set_odr_raw(dev, accel_odr);
		if (ret < 0) {
			LOG_ERR("Failed to resume accelerometer");
			break;
		}
		gyro_odr = lsm6ds3_freq_to_odr_val(data->gyro_freq);
		ret = lsm6ds3_gyro_set_odr_raw(dev, gyro_odr);
		if (ret < 0) {
	...
	case PM_DEVICE_ACTION_SUSPEND:
		 ...
		 */
		ret = data->hw_tf->update_reg(dev, LSM6DS3_REG_CTRL1_XL,
					      LSM6DS3_MASK_CTRL1_XL_ODR_XL, 0);
		if (ret < 0) {
			LOG_ERR("Failed to suspend accelerometer");
			break;
		}
		/* Set gyro ODR to power-down */
		ret = data->hw_tf->update_reg(dev, LSM6DS3_REG_CTRL2_G,
					      LSM6DS3_MASK_CTRL2_G_ODR_G, 0);
		if (ret < 0) {
...
#define LSM6DS3_DEFINE(inst)						\
	COND_CODE_1(DT_INST_ON_BUS(inst, spi),				\
		    (LSM6DS3_DEFINE_SPI(inst)),				\
		    (LSM6DS3_DEFINE_I2C(inst)))
