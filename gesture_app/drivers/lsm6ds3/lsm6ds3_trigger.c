...
static void lsm6ds3_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
...
int lsm6ds3_init_interrupt(const struct device *dev)
{
	...
	gpio_init_callback(&drv_data->gpio_cb,
			   lsm6ds3_gpio_callback, BIT(config->int_gpio.pin));
	...
	if (drv_data->hw_tf->update_reg(dev,
			       LSM6DS3_REG_INT1_CTRL,
			       LSM6DS3_MASK_INT1_CTRL_DRDY_XL |
			       LSM6DS3_MASK_INT1_CTRL_DRDY_G,
			       BIT(LSM6DS3_SHIFT_INT1_CTRL_DRDY_XL) |
			       BIT(LSM6DS3_SHIFT_INT1_CTRL_DRDY_G)) < 0) {
